...
public class Offer12任意两个整数的加法 {
    ...
    public static char[] add(char[] num1, char[] num2) {
        ...
        } else//3-一正一负相加 异号
        {
            ...
            if (max == 0) {
                ...
                char[] num1DeleteHead0 = deleteHead0(deletePlusMinus(num1));
                char[] num2DeleteHead0 = deleteHead0(deletePlusMinus(num2));
                if (max == 1)//num1大,位数多  num1=-9 num2=3;
                {
                    if (num1[0] == '-')//num1大，且大值是负数，那结果一定是负数（当做两个正数相减）
                    {
                        char[] result = subtractOfPositiveNum(num1DeleteHead0, num2);
                        char[] finalResult = new char[result.length + 1];
                        finalResult[0] = '-';
                        System.arraycopy(result, 0, finalResult, 1, result.length);
                        return finalResult;
                    } else//num1=9,num2=-3//就是9-3 结果是正数
                        return subtractOfPositiveNum(num1DeleteHead0, num2DeleteHead0);
                } else //num2大 num1=3 num2=-9
                {
                    if (num2[0] == '-')//num2大，且num2是正值，num1是负值 结果是负数
                    {
                        char[] result = subtractOfPositiveNum(num2DeleteHead0, num1DeleteHead0);
                        char[] finalResult = new char[result.length + 1];
                        result[0] = '-';
                        System.arraycopy(result, 0, finalResult, 1, result.length);
                        return finalResult;
                    } else //num2大，但num2是正值
                        return subtractOfPositiveNum(num2DeleteHead0, num1DeleteHead0);
                }
            }
        }
    }
    ...
    //减，两个正数相减
    public static char[] subtractOfPositiveNum(char[] num1, char[] num2) {//num1：正树，num2-负数 1大 2小
    ...
    //检查一个数组，存储的是正数，还是负数。正值返回1 负值返回-1  数值错误返回0
